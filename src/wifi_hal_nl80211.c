...
int process_mgmt_frame(struct nl_msg *msg, void *arg)
{
    ...
    if (gnlh->cmd != NL80211_CMD_FRAME) {
        wifi_hal_dbg_print("%s:%d: Unknown event\n", __func__, __LINE__);
        return NL_SKIP;
    }

    if ((attr = tb[NL80211_ATTR_FRAME]) == NULL) {
        wifi_hal_dbg_print("%s:%d: frame attribute absent ... dropping\n", __func__, __LINE__);
        return NL_SKIP;
    } 
    ...
    case WLAN_FC_STYPE_REASSOC_REQ:
        ...
        memcpy(associated_dev.cli_MACAddress, sta, sizeof(mac_address_t));
    ...
    case WLAN_FC_STYPE_ACTION:
        ...
        wifi_hal_dbg_print("%s:%d: Received action frame from: %s\n", __func__, __LINE__, 
                           to_mac_str(sta, sta_mac_str));
        break;
    ...
    case WLAN_FC_STYPE_DISASSOC:
        ...
        for (int i = 0; i < callbacks->num_disassoc_cbs; i++) {
            if (callbacks->disassoc_cb[i] != NULL) {
                callbacks->disassoc_cb[i](vap->vap_index, to_mac_str(sta, sta_mac_str), 0);
            }
        }
        break;
...
void recv_data_frame(wifi_interface_info_t *interface)
{
    ...
    if (memcmp(eth_hdr->dest, interface->mac, sizeof(mac_address_t)) == 0) {
      ...
      //  dir = wifi_direction_downlink;     
        ...
        // drop
        wifi_hal_dbg_print("%s:%d: dropping eapol frame\n", __func__, __LINE__);
        return;
    }
...
void *nl_recv_func(void *arg)
{
    ...
    while (1) {
        ...
        if (ret < 0) {
            wifi_hal_dbg_print("%s:%d:selct error\n", __func__, __LINE__);
            if (errno == EINTR) {
                continue;    
            } else {
                return NULL;
        ...
        if (FD_ISSET(priv->nl_event_fd, &priv->drv_rfds)) {
            ...
            if (res < 0) {
                wifi_hal_dbg_print("%s:%d: %s->nl_recvmsgs failed: %d\n", __func__, __LINE__, __func__, res);
            }
        }
...
struct nl_handle *nl_create_handle(struct nl_cb *cb, const char *dbg)
{
    ...
    if (handle == NULL) {
        wifi_hal_dbg_print("%s:%d: Failed to allocate netlink callbacks (%s)\n", __func__, __LINE__, dbg);
        return NULL;
    }
    ...
    if (genl_connect((struct nl_sock *)handle)) {
        wifi_hal_dbg_print("%s:%d: Failed to connect to generic netlink (%s)\n", __func__, __LINE__, dbg);
...
int get_vap_state(const char *ifname, short *flags)
{
    ...
    if ((fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        wifi_hal_dbg_print("socket error %s\n", strerror(errno));
        return -1;
    }
...
int nl80211_remove_from_bridge(const char *if_name)
{
    ...
    if (nl_connect(sk, NETLINK_ROUTE)) {
        wifi_hal_dbg_print("Unable to connect socket");
        nl_socket_free(sk);
        return -1;
    }

    if (rtnl_link_alloc_cache(sk, AF_UNSPEC, &link_cache)) {
        wifi_hal_dbg_print("Unable to allocate cache");
        nl_socket_free(sk);
        return -1;
    }
    ...
    if (rtnl_link_release(sk, device)) {
        wifi_hal_dbg_print("%s:%d:Unable to release interface:%s \n", __func__, __LINE__, if_name);
...
int nl80211_create_bridge(const char *if_name, const char *br_name)
{
    ...
    if (rtnl_link_enslave(sk, bridge, device)) {
        wifi_hal_dbg_print("%s:%d:Unable to enslave interface:%s to bridge:%s\n", __func__, __LINE__, if_name, br_name);
        nl_cache_free(link_cache);
        nl_socket_free(sk);
        return -1;
    }
...
int nl80211_interface_enable(const char *ifname, bool enable)
{
    ...
    if (get_vap_state(ifname, &flags) < 0) {
        wifi_hal_dbg_print("%s:%d coul not get state of interface %s\n", __func__, __LINE__, ifname);
        return -1;
    }
...
static struct hostapd_hw_modes *phy_info_freqs(wifi_radio_info_t *radio, struct nlattr *tb, enum nl80211_band *nlband)
{
        ...
        if (tb_freq[NL80211_FREQUENCY_ATTR_WMM]) {
            nla_for_each_nested(nl_wmm, tb_freq[NL80211_FREQUENCY_ATTR_WMM], rem_wmm) {
                if (nla_parse_nested(tb_wmm, NL80211_WMMR_MAX, nl_wmm, wmm_policy)) {
                    wifi_hal_dbg_print("%s:%d:Failed to parse WMM rules attribute\n", __func__, __LINE__);
                    break;
                }

                if (!tb_wmm[NL80211_WMMR_CW_MIN] || !tb_wmm[NL80211_WMMR_CW_MAX] || !tb_wmm[NL80211_WMMR_AIFSN] || !tb_wmm[NL80211_WMMR_TXOP]) {
                    wifi_hal_dbg_print("%s:%d: Channel is missing WMM rule attribute\n", __func__, __LINE__);
                    break;
                }

                ac = nl_wmm->nla_type;
                if (ac < 0 || ac >= WMM_AC_NUM) {
                    wifi_hal_dbg_print("%s:%d: Invalid AC value %d", __func__, __LINE__, ac);
                    break;
                }
    ...
    //wifi_hal_dbg_print("%s:%d: Freq Band: %s for radio:%d num channels: %d channels: ", __func__, __LINE__, 
    //    wifi_freq_bands_to_string(freq_band), radio->index, mode->num_channels);
    
    for (i = 0; i < channels->num_channels; i++) {
        ieee80211_freq_to_chan(chan->freq, (u8 *)&channels->channels_list[i]);
        //wifi_hal_dbg_print("%d ", channels->channels_list[i]);
        chan++;
    }
    //wifi_hal_dbg_print("\n");
...
static int wiphy_dump_handler(struct nl_msg *msg, void *arg)
{
    ...
    if (tb[NL80211_ATTR_WIPHY]) {
        radio->index = nla_get_u32(tb[NL80211_ATTR_WIPHY]);
...
static int wiphy_get_info_handler(struct nl_msg *msg, void *arg)
{
    ...
    if (tb[NL80211_ATTR_WIPHY_BANDS] != NULL) {
        ...
        wifi_hal_dbg_print("%s:%d: Bands attribute not present in radio index:%d\n", __func__, __LINE__, radio->index);
    }
    if (tb[NL80211_ATTR_INTERFACE_COMBINATIONS]) {
        nla_for_each_nested(nl_combi, tb[NL80211_ATTR_INTERFACE_COMBINATIONS], rem_combi) {
            ...
            if ((nla_parse_nested(tb_comb, MAX_NL80211_IFACE_COMB, nl_combi, iface_combination_policy) != 0 ) || !tb_comb[NL80211_IFACE_COMB_MAXNUM])
                wifi_hal_dbg_print("%s:%d: Failed to parse interface combinations for radio index:%d\n", __func__, __LINE__, radio->index);
            else {
        ...
        wifi_hal_dbg_print("%s:%d: Interface combinations attribute not present in radio index:%d\n", __func__, __LINE__, radio->index);
    }
    return NL_OK;
}
    ...
    if (tb[NL80211_ATTR_WIPHY]) {
        ...
        if (radio != NULL && tb[NL80211_ATTR_IFNAME]) {
            ...
            if (tb[NL80211_ATTR_MAC]) {
                ...
                memcpy(vap->u.bss_info.bssid, nla_data(tb[NL80211_ATTR_MAC]), nla_len(tb[NL80211_ATTR_MAC]));
            }
    
    
            if (set_interface_properties(nla_get_u32(tb[NL80211_ATTR_WIPHY]), interface) != 0) {
                wifi_hal_dbg_print("%s:%d: Could not map interface name to index\n", __func__, __LINE__);
                free(interface);
                return NL_SKIP;
            }
...
int init_nl80211()
{
    ...
    if (g_wifi_hal.nl_cb == NULL) {
        wifi_hal_dbg_print("%s:%d: Failed to allocate netlink callbacks\n", __func__, __LINE__);
        return -1;
    }
    ...
    if (g_wifi_hal.nl80211_id < 0) {
        wifi_hal_dbg_print("%s:%d: generic netlink not found\n", __func__, __LINE__);
        nl_cb_put(g_wifi_hal.nl_cb);
        return -1;
    }
    ...
    if (ret < 0) {
        wifi_hal_dbg_print("%s:%d: Could not add multicast membership for scan events: %d (%s)\n", __func__, __LINE__,           
               ret, strerror(-ret));
    ...
    if (ret < 0) {
        wifi_hal_dbg_print("%s:%d: Could not add multicast membership for mlme events: %d (%s)\n", __func__, __LINE__,
               ret, strerror(-ret));
        nl_destroy_handles(&g_wifi_hal.nl);
        nl_cb_put(g_wifi_hal.nl_cb);
        return -1;
    ...
    if (ret < 0) {
        wifi_hal_dbg_print("%s:%d: Could not add multicast membership for regulatory events: %d (%s)\n", 
    ...
    if (ret < 0) {
        wifi_hal_dbg_print("%s:%d: Could not add multicast membership for vendor events: %d (%s)\n", 
                __func__, __LINE__, ret, strerror(-ret));
    }
    ...
    if (g_wifi_hal.link_fd  > 0) {
        ...
        if (bind(g_wifi_hal.link_fd, (struct sockaddr*)&local, sizeof(local)) <0) {
            wifi_hal_dbg_print("%s:%d: Socket bind failed \n", __func__, __LINE__);
            close(g_wifi_hal.link_fd);
            return -1;
        }
    } else {
        wifi_hal_dbg_print("%s:%d: socket creation failed for link_fd\n", __func__, __LINE__);
        return -1;
    }
    ...
    for (i = 0; i < g_wifi_hal.num_radios; i++) {
        ...
        if (send_and_recv(g_wifi_hal.nl_cb, g_wifi_hal.nl, msg, wiphy_get_info_handler, &g_wifi_hal, NULL, NULL)) {
            return -1;
        }
...
int nl80211_enable_ap(wifi_interface_info_t *interface, bool enable)
{
    ...
    if (enable) {
        msg = nl80211_drv_cmd_msg(g_wifi_hal.nl80211_id, NULL, 0, NL80211_CMD_START_AP);
    } else {
    ...
    if ((ret = send_and_recv(g_wifi_hal.nl_cb, g_wifi_hal.nl, msg, ap_enable_handler, &g_wifi_hal, NULL, NULL))) {
        wifi_hal_dbg_print("%s:%d: Error stopping/starting ap: %s\n", __func__, __LINE__, strerror(-ret));
        return RETURN_ERR;
    }
...
int nl80211_init_primary_interfaces()
{
    ...
    for (i = 0; i < g_wifi_hal.num_radios; i++) {
        ...
        if (primary_interface == NULL) {
            wifi_hal_dbg_print("%s:%d: Error updating dev:%d no primary interfaces exist\n", __func__, __LINE__, radio->index);
            return -1;
        }
        ...
        if (interface == NULL) {
            wifi_hal_dbg_print("%s:%d: Error updating dev:%d no private vap interfaces exist\n", __func__, __LINE__, radio->index);
            return -1;
        }
        ...
        if ((ret = send_and_recv(g_wifi_hal.nl_cb, g_wifi_hal.nl, msg, interface_info_handler, &g_wifi_hal, NULL, NULL))) {
            wifi_hal_dbg_print("%s:%d: Error updating %s interface on dev:%d error: %s\n",
...
int nl80211_update_wiphy(wifi_radio_info_t *radio)
{
    ...
    if (interface == NULL) {
        wifi_hal_dbg_print("%s:%d: Error updating dev:%d no interfaces exist\n", __func__, __LINE__, radio->index);
        return -1;
    }
    ...
    if ((ret = send_and_recv(g_wifi_hal.nl_cb, g_wifi_hal.nl, msg, wiphy_set_info_handler, &g_wifi_hal, NULL, NULL))) {
        wifi_hal_dbg_print("%s:%d: Error updating dev:%d error: %s\n",
    ...
    wifi_hal_dbg_print("%s:%d: Updating dev:%d successful\n",
            __func__, __LINE__, radio->index);
   ...
   // radio = get_radio_by_index(vap->radio_index);
    ...
    for (i = 0; i < sizeof(stypes)/sizeof(int); i++) {
        ...
        if ((ret = send_and_recv(interface->nl_cb, interface->nl_event, msg, mgmt_frame_register_handler, interface, NULL, NULL))) {
            wifi_hal_dbg_print("%s:%d: Error registering for management frames on interface %s error: %s\n",
...
int nl80211_update_interface(wifi_interface_info_t *interface)
{
    ...
    if (vap->vap_mode == wifi_vap_mode_ap) {
        ...
        if ((ret = send_and_recv(g_wifi_hal.nl_cb, g_wifi_hal.nl, msg, interface_info_handler, &g_wifi_hal, NULL, NULL))) {
            printf("%s:%d: Error updating %s interface on dev:%d error: %s\n",
                        __func__, __LINE__, interface->name, radio->index, strerror(-ret));
            return -1;
        }

        printf("%s:%d: Updating %s interface on dev:%d to type: NL80211_IFTYPE_AP successful\n",
...
int nl80211_create_interface(wifi_radio_info_t *radio, wifi_vap_info_t *vap, wifi_interface_info_t **interface)
{
    ...
    if ((ret = send_and_recv(g_wifi_hal.nl_cb, g_wifi_hal.nl, msg, interface_info_handler, &g_wifi_hal, NULL, NULL))) {
        wifi_hal_dbg_print("%s:%d: Error creating %s interface on dev:%d error: %s\n", __func__, __LINE__, 
...
int nl80211_create_interfaces(wifi_radio_info_t *radio, wifi_vap_info_map_t *map)
{
    ...
    for (i = 0; i < map->num_vaps; i++) {
        ...
        if (nl80211_create_interface(radio, vap, &interface) != 0) {
            wifi_hal_dbg_print("%s:%d:interface for vap index:%d create failed\n", 
                __func__, __LINE__, vap->vap_index);
            return -1;
        }
    }
...
//    gnlh = nlmsg_data(nlmsg_hdr(msg));
    ...
    if (callbacks->scan_result_callback == NULL) {
        wifi_hal_dbg_print("%s:%d: Scan results call back not registered\n", __func__, __LINE__);
        return NL_SKIP;
    }
...
int nl80211_get_scan_results(wifi_interface_info_t *interface)
{
    ...
    wifi_hal_dbg_print("%s:%d: Scan command failed: ret=%d (%s)\n", __func__, __LINE__,
                            ret, strerror(-ret));
...
int nl80211_disconnect_sta(wifi_interface_info_t *interface)
{
    ...
    wifi_hal_dbg_print("%s:%d: disconnect command failed: ret=%d (%s)\n", __func__, __LINE__,
                      ret, strerror(-ret));
...
int nl80211_connect_sta(wifi_interface_info_t *interface)
{
    ...
    if (security->encr == wifi_encryption_aes) {
        ...
        wifi_hal_dbg_print("%s:%d:Invalid encryption mode:%d in wifi_hal_connect\n", __func__, __LINE__, security->encr);
    }
    ...
    switch (security->mode) {
        ...
        case wifi_security_mode_wpa3_transition:
            ...
            break;
        default:
             wifi_hal_dbg_print("%s:%d:Invalid security mode: %d in wifi_hal_connect\r\n", __func__, __LINE__, security->mode);
    ...
    if (security->mode != wifi_security_mode_none) {
        if ((ret = wpa_write_rsn_ie(&wpa_conf, pos, rsn_ie + sizeof(rsn_ie) - pos, NULL)) < 0) {
            wifi_hal_dbg_print("%s:%d Failed to build RSN %d\r\n", __func__, __LINE__, ret);
            return ret;
        }
        else {
            pos += ret;
    ...
    wifi_hal_dbg_print("%s:%d: connect command failed: ret=%d (%s)\n", __func__, __LINE__,
                      ret, strerror(-ret));
...
static int scan_info_handler(struct nl_msg *msg, void *arg)
{
    ...
    if (vap->vap_mode != wifi_vap_mode_sta) {
        wifi_hal_dbg_print("%s:%d:not sta mode\n", __func__, __LINE__);
        return NL_SKIP;
    }
    ...
    if (tb[NL80211_ATTR_BSS] == NULL) {
        wifi_hal_dbg_print("%s:%d:bss attribute not present\n", __func__, __LINE__);
        return NL_SKIP;
    }

    if (nla_parse_nested(bss, NL80211_BSS_MAX, tb[NL80211_ATTR_BSS], bss_policy) != 0) {
        wifi_hal_dbg_print("%s:%d:nested bss attribute not present\n", __func__, __LINE__);
        return NL_SKIP;
    }
    ...
    if (get_ie_by_eid(WLAN_EID_SSID, (unsigned char *)ie, len, (unsigned char **)&ie_ssid, &ie_ssid_len) == true) {
        memcpy(scan_info->ssid, ie_ssid->value, ie_ssid_len - sizeof(ieee80211_tlv_t));
    }
    ...
    if (strcmp(scan_info->ssid, vap->u.sta_info.ssid) == 0) {
        wifi_hal_dbg_print("%s:%d:backhaul BSSID:%s for ssid:%s\n", __func__, __LINE__, 
            to_mac_str(bssid, bssid_str), scan_info->ssid);
        memcpy(vap->u.sta_info.bssid, bssid, sizeof(bssid_t));
    }
#if 0
    scan_info->ie_len = len;
    if (len != 0) {
        os_memcpy(scan_info + sizeof(wifi_bss_info_t), ie, len);
    }
#endif
    return NL_SKIP;
}
...
int nl80211_update_beacon_params(wifi_interface_info_t *interface)
{
    ...
    wifi_hal_dbg_print("%s:%d: bacon get command failed:%s\n", __func__, __LINE__, strerror(-ret));
                                  ...
                                  size_t csa_offs_len)
{
    ...
    msg = NULL;
    if (ret) {
        wifi_hal_dbg_print("nl80211: Frame command failed: ret=%d (%s) (freq=%u )\n",
    ...
    const u8 *addr, int deauth, u16 reason_code)
{
    ...
    wifi_hal_dbg_print("nl80211: sta_remove -> DEL_STATION %s %s --> %d (%s)\n",
...
int wifi_drv_flush(void *priv)
{
    ...
    ret = send_and_recv(g_wifi_hal.nl_cb, g_wifi_hal.nl, msg, NULL, NULL, NULL, NULL);
    if (ret) {
        wifi_hal_dbg_print("nl80211: Station flush failed: ret=%d (%s)", ret, strerror(-ret));
    }
    return ret;
}
                        ...
                        unsigned int flags_and)
{
    ...
    wifi_hal_dbg_print("nl80211: Set STA flags - ifname=%s addr=%s" 
    ...
    size_t data_len, int encrypt, const u8 *own_addr, u32 flags)
{
    ...
    wifi_hal_dbg_print("%s:%d: Sending eapol to sta:%s on interface:%s\n", __func__, __LINE__, 
...
int wifi_drv_sta_add(void *priv, struct hostapd_sta_add_params *params)
{
    ...
    wifi_hal_dbg_print("nl80211: %s STA %s" , params->set ? "Set" : "Add", to_mac_str(params->addr, mac_str));
    ...
    msg = NULL;
    if (ret) {
        wifi_hal_dbg_print("nl80211: NL80211_CMD_%s_STATION "
...
int wifi_drv_set_acl(void *priv, struct hostapd_acl_params *params)
{
    ...
    ret = send_and_recv(g_wifi_hal.nl_cb, g_wifi_hal.nl, msg, NULL, NULL, NULL, NULL);
    if (ret) {
        wifi_hal_dbg_print("nl80211: Failed to set MAC ACL: %d (%s)",
                           ret, strerror(-ret));
    }
                   ...
                   struct wpa_driver_ap_params *params)
{
    ...
    case BEACON_RATE_LEGACY:
        if (!(flags & WPA_DRIVER_FLAGS_BEACON_RATE_LEGACY)) {
            wifi_hal_dbg_print("nl80211: Driver does not support setting Beacon frame rate (legacy)\n");
            return -1;
        }
    ...
    case BEACON_RATE_HT:
        if (!(flags & WPA_DRIVER_FLAGS_BEACON_RATE_HT)) {
            wifi_hal_dbg_print("nl80211: Driver does not support setting Beacon frame rate (HT)\n");
            return -1;
        }
    ...
    case BEACON_RATE_VHT:
        if (!(flags & WPA_DRIVER_FLAGS_BEACON_RATE_VHT)) {
            wifi_hal_dbg_print("nl80211: Driver does not support setting Beacon frame rate (VHT)\n");
            return -1;
        }
...
int wifi_drv_set_ap(void *priv, struct wpa_driver_ap_params *params)
{
    ...
    wifi_hal_dbg_print("%s:%d:Enter, interface name:%s vap index:%d radio index:%d\n", __func__, __LINE__,
        interface->name, vap->vap_index, radio->index);

    drv = &radio->driver_data;

    if ((msg = nl80211_drv_cmd_msg(g_wifi_hal.nl80211_id, interface, 0, NL80211_CMD_NEW_BEACON)) == NULL) {
        wifi_hal_dbg_print("%s:%d: Failed to create message\n", __func__, __LINE__);
        return -1;
    }
    ...
    ret = send_and_recv(g_wifi_hal.nl_cb, g_wifi_hal.nl, msg, beacon_info_handler, &g_wifi_hal, NULL, NULL);
    if (ret != 0) {
        wifi_hal_dbg_print("%s:%d: Failed to set beacon parameter for interface: %s error: %s\n", __func__, __LINE__, interface->name, strerror(-ret));
        return -1;
    }
...
int wifi_drv_set_supp_port(void *priv, int authorized)
{
    ...
    {
        wifi_hal_dbg_print("Failed to create command SET_STATION\n");
        nlmsg_free(msg);
        return -ENOBUFS;
    }
    ...
    wifi_hal_dbg_print("%s:%d: set supp port command failed: ret=%d (%s)\n", __func__, __LINE__,
        ret, strerror(-ret));
...
int wifi_drv_set_operstate(void *priv, int state)
{
    ...
    wifi_hal_dbg_print("%s:%d: Enter, interface:%s bridge:%s driver operation state:%d\n", 
    ...
    if ((vap->vap_mode == wifi_vap_mode_ap) && (nl80211_register_mgmt_frames(interface) != 0)) {
        wifi_hal_dbg_print("%s:%d: Failed to register for management frames\n", __func__, __LINE__);
        return -1;
    }
    ...
    if (sock_fd < 0) {
        wifi_hal_dbg_print("%s:%d: Failed to open raw socket on bridge: %s\n", __func__, __LINE__, vap->bridge_name);
        return -1;
    }
    ...
    if (bind(sock_fd, (struct sockaddr *)&sockaddr, sizeof(sockaddr)) < 0) {
        wifi_hal_dbg_print("%s:%d: Error binding to interface, err:%d\n", __func__, __LINE__, errno);
        close(sock_fd);
        return -1;
    }
    ...
    wifi_hal_dbg_print("%s:%d: Exit, interface:%s bridge:%s driver configured for 802.11\n", 
                    ...
                    size_t seq_len, const u8 *key, size_t key_len)
{
    ...
    if ((ret = send_and_recv(g_wifi_hal.nl_cb, g_wifi_hal.nl, msg, NULL, (void *)-1, NULL, NULL))) {
        wifi_hal_dbg_print("%s:%d: Failed to set key: %s\n", __func__, __LINE__, strerror(-ret));
        return -1;
    }

    wifi_hal_dbg_print("%s:%d:key set success\n", __func__, __LINE__);
