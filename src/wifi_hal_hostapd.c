...
void init_radius_config(wifi_interface_info_t *interface)
{
    if (!interface->vap_initialized) {
        ...
        char *config_methods = (char *)malloc(512);
        memset(config_methods, '\0', 512);
...
void init_oem_config(wifi_interface_info_t *interface)
{
    ...
    conf->device_name = &interface->device_name;
    conf->manufacturer = &interface->manufacturer;
    conf->model_name = &interface->model_name;
    conf->model_number = &interface->model_number;
    conf->serial_number = &interface->serial_number;
    strcpy(conf->device_type, "Device Type");
    conf->friendly_name = &interface->friendly_name;
    conf->manufacturer_url = &interface->manufacturer_url;
    conf->model_description = &interface->model_description;
    conf->model_url = &interface->model_url;
    conf->ap_vlan = interface->vlan;

    strcpy(interface->device_name, "Device Name");
    strcpy(interface->manufacturer, "Manufacturer");
    strcpy(interface->model_name, "Model Name");
    strcpy(interface->model_number, "Model Number");
    strcpy(interface->serial_number, "Serial Number");
    strcpy(interface->friendly_name, "Friendly Name");
    strcpy(interface->manufacturer_url, "Manufacturer URL");
    strcpy(interface->model_description, "Model Description");
    strcpy(interface->model_url, "Model URL");
#endif
#endif
}
...
//Not Defined
    ...
    if (hapd->drv_priv == NULL) {
        wifi_hal_dbg_print("%s:%d:Invalid driver params\n", __func__, __LINE__);
        return RETURN_ERR;
    }
    return RETURN_OK;
}
...
int update_security_config(wifi_vap_security_t *sec, struct hostapd_bss_config *conf)
{
    ...
    default:
        wifi_hal_dbg_print("%s:%d:Invalid encryption mode in VAP setting\n", 
                           __func__, __LINE__);
        break;
    }
    ...
    if (conf->ieee802_1x) {
        ...
        if (conf->ieee802_1x && !conf->eap_server && !conf->radius->auth_servers) {
            wifi_hal_dbg_print("%s:%d:Invalid 802.1x configuration in VAP setting\n", __func__, __LINE__);
            return RETURN_ERR;
        }

        if (sec->u.radius.ip == 0) {
            wifi_hal_dbg_print("%s:%d:Invalid radius server IP configuration in VAP setting\n", __func__, __LINE__);
            return RETURN_ERR;
        }
...
void wps_enum_to_string(unsigned int methods, char *str) {

    int itr = 0, size = 0, len = 512;
...
int update_hostap_bss(wifi_interface_info_t *interface)
{
    ...
    if (update_security_config(&vap->u.bss_info.security, conf) == -1) {
        wifi_hal_dbg_print("%s:%d:update_security_config failed \n", __func__, __LINE__);
        return RETURN_ERR;
    }
#if 0
#ifdef CONFIG_IEEE80211W
    ...
    if (conf->access_network_type < 0 || conf->access_network_type > 15) {
        wifi_hal_dbg_print("%s:%d:Invalid access network setting in VAP setting\n", __func__, __LINE__);
        return RETURN_ERR;
    }
...
#ifdef CONFIG_WPS
    if (conf->wps_state) {
        wps_enum_to_string(vap->u.bss_info.wps.methods, (char *)conf->config_methods);
...
int update_hostap_iface(wifi_interface_info_t *interface)
{
    ...
    wifi_hal_dbg_print("%s:%d: Interface: %s band: %d mode:%p has %d rates\n", __func__, __LINE__, 
...
*/
    ...
    wifi_hal_dbg_print("%s:%d:interface name:%s country:%s op class:%d channel:%d frequency:%d\n", __func__, __LINE__, 
...
int update_hostap_interfaces(wifi_radio_info_t *radio)
{
    ...
    if (radio == NULL) {
        wifi_hal_dbg_print("%s:%d:wifi_radio_info is NULL\n", __func__, __LINE__);
        return RETURN_ERR;
    }
...
//Not defined
    ...
    if (hostapd_config_check(iconf, 1) < 0) {
        wifi_hal_dbg_print("%s:%d:Invalid config params\n", __func__, __LINE__);
        return RETURN_ERR;
    }
...
static int wpa_sm_sta_ether_send(void *ctx, const u8 *dest, u16 proto, const u8 *buf, size_t len)
{
    ...
    if (ret < 0) {
        wifi_hal_dbg_print("%s:%d: error:%s\n", __func__, __LINE__, strerror(errno));
    } else {
        //my_print_hex_dump(len + sizeof(struct ieee8023_hdr), buff);
        wifi_hal_dbg_print("%s:%d: send eapol key to:%s success, length of payload:%d\n", __func__, __LINE__, 
...
static int wpa_sm_sta_get_beacon_ie(void *ctx)
{
    ...
    while (bss != NULL) {
        if (memcmp(backhaul->bssid, bss->bssid, sizeof(bssid_t)) == 0) {
            if (bss->ie_len) {
                ie = wpa_bss_get_vendor_ie((unsigned char *)(bss + sizeof(wifi_bss_info_t)), bss->ie_len, WPA_IE_VENDOR_TYPE);

                if (wpa_sm_set_ap_wpa_ie(interface->u.sta.wpa_sm, ie, ie ? 2 + ie[1] : 0)) {
                    ret = -1;
                }

                ie = get_ie((unsigned char *)(bss + sizeof(wifi_bss_info_t)), bss->ie_len, WLAN_EID_RSN);

                if (wpa_sm_set_ap_rsn_ie(interface->u.sta.wpa_sm, ie, ie ? 2 + ie[1] : 0)) {
                    ret = -1;
                }
...
void update_wpa_sm_params(wifi_interface_info_t *interface)
{
    ...
    if (interface->u.sta.wpa_sm == NULL) {
        ...
        if (ctx == NULL) {
            wifi_hal_dbg_print("%s:%d: Failed to alloc ctx\n", __func__, __LINE__);
            return;
        }
        ctx->ctx = interface;
    ...
    if (sec->mode == wifi_security_mode_wpa2_personal)
        ...
        wifi_hal_dbg_print("Unsupported security mode\n");
        return;
    }
                                            ...
                                            size_t len)
{
    ...
    if (ret < 0) {
        ...
        wifi_hal_dbg_print("%s:%d: send eapol to:%s success, length of payload:%d\n", __func__, __LINE__, 
	...
	interface->u.sta.wpa_sm->eapol = eapol_sm_init(ctx);
        ...
        if (sec->mode == wifi_security_mode_wpa2_enterprise) {
	        ...
	        case WIFI_EAP_TYPE_PWD:
	    	    ...
	    	    break;
	        default:
		    wifi_hal_dbg_print("Unsupported EAP method\n");
		    return;
            }
